...
This took 132m26.055s. We will want to shuffle this on Dataflow later, so I will
...
My run took about 36m48.815s on Dataflow.
...
In my run, it wrote to 364 shards:
...
```
35100   96542.0 0.99896929109
```

This means the model checkpoint that performs the best on the validation set is
`${TRAINING_DIR}/model.ckpt-35100`. Based on this result, a few thoughts came
into mind:
...
( time sudo docker run \
    ...
    --examples "${OUTPUT_DIR}/test_set.no_label.tfrecord@${N_SHARDS}.gz" \
    --checkpoint "${TRAINING_DIR}/model.ckpt-35100" \
...
----- | ----- | ---- | ---- | -------- | --------- | ---------
INDEL | 9799  | 224  | 139  | 0.977651 | 0.986539  | 0.982075
SNP   | 66171 | 66   | 52   | 0.999004 | 0.999215  | 0.999109
