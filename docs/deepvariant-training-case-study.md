...
*   Indel F1 `95.7946%` --> `98.3305%`
*   SNP F1: `99.8755%` --> `99.9155%`
...
GCS_PRETRAINED_WGS_MODEL="${BUCKET}/models/DeepVariant/1.10.0/checkpoints/wgs/deepvariant.wgs.ckpt"
...
curl -O https://raw.githubusercontent.com/google/deepvariant/r1.10/scripts/install_nvidia_docker.sh
...
wget https://raw.githubusercontent.com/google/deepvariant/r1.10/tools/shuffle_tfrecords_beam.py -O ${SHUFFLE_SCRIPT_DIR}/shuffle_tfrecords_beam.py
```
...
pip3 install "setuptools<82" "protobuf==4.21.9" "tensorflow==2.16.1"
pip3 install apache_beam[gcp]==2.71.0
```
...
```bash
curl https://raw.githubusercontent.com/google/deepvariant/r1.10/deepvariant/dv_config.py > dv_config.py
```
...
We tested with 1 GPU - it took 93m16.476s.
...
model.example_info.json file. Because we didn't change any channels,
for convenience, I'll copy over the WGS one:

```bash
gsutil cp \
  gs://deepvariant/models/DeepVariant/1.10.0/checkpoints/wgs/model.example_info.json \
  ${TRAINING_DIR}/checkpoints/ema/
```
...
```
E external/local_xla/xla/stream_executor/cuda/cuda_driver.cc:282] failed call to cuInit: CUDA_ERROR_NO_DEVICE: no CUDA-capable device is detected
```
...
INDEL    ALL        10023      9823       200        19436       139       9074     91     25       0.980046          0.986586        0.466866         0.983305                     NaN                     NaN                   1.547658                   2.103035
INDEL   PASS        10023      9823       200        19436       139       9074     91     25       0.980046          0.986586        0.466866         0.983305                     NaN                     NaN                   1.547658                   2.103035
  SNP    ALL        66237     66180        57        78319        55      12047     10      3       0.999139          0.999170        0.153820         0.999155                2.284397                2.197102                   1.700387                   1.800336
  SNP   PASS        66237     66180        57        78319        55      12047     10      3       0.999139          0.999170        0.153820         0.999155                2.284397                2.197102                   1.700387                   1.800336
```
...
| ----- | -------- | -------- | -------- | ------------- | ---------------- | --------------- |
| INDEL | 9823     | 200      | 139      | 0.980046      | 0.986586         | 0.983305        |
| SNP   | 66180    | 57       | 55       | 0.999139      | 0.99917          | 0.999155        |
...
| Type  | TRUTH.TP | TRUTH.FN | QUERY.FP | METRIC.Recall | METRIC.Precision | METRIC.F1_Score |
| ----- | -------- | -------- | -------- | ------------- | ---------------- | --------------- |
| INDEL | 9725     | 298      | 579      | 0.970268      | 0.945933         | 0.957946        |
| SNP   | 66157    | 80       | 85       | 0.998792      | 0.998718         | 0.998755        |
